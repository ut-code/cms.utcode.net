...
async function seed() {
  ...
  const members = await db.insert(schema.member).values(memberData).returning();
  ...
  const articles = await db.insert(schema.article).values(articleData).returning();
  ...
  await db.insert(schema.projectMember).values(projectMemberData);
